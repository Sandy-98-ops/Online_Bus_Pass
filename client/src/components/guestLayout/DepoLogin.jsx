...
import { depoLogin } from '../../utils/api';

const DepoLogin = () => {
    const cookies = new Cookies();

    const [loginErrors, setLoginErrors] = useState({});
    const [loginData, setLoginData] = useState({ depoEmail: '', password: '' });
    ...
    const validateLogin = () => {
        const errors = {};
        if (!loginData.depoEmail) errors.depoEmail = 'Email is required';
    ...
    const handleLoginSubmit = async (e) => {
        ...
        setLoginErrors({});
        try {
            const response = await depoLogin(loginData);
            if (response.ok) {
                const result = await response.json();
                const depo = result.depo;
                cookies.set('depoData',
                    JSON.stringify(depo), { path: '/' })

                navigate('/depo'); // Redirect to admin layout
        ...
        <div >
            <Container className='container-2'>
                ...
                <div className="form form-1">
                    <h2 className='pb-4'>Depo Login</h2>
                    ...
                    <Form onSubmit={handleLoginSubmit}>
                        <Form.Group controlId="formEmail">
                            <Form.Control
                                ...
                                className={`mb-3 ${loginErrors.depoEmail ? 'is-invalid' : ''}`}
                                value={loginData.depoEmail}
                                onChange={handleLoginChange}
                            />
                            {loginErrors.depoEmail && <p className="invalid-feedback">{loginErrors.depoEmail}</p>}
                        </Form.Group>
